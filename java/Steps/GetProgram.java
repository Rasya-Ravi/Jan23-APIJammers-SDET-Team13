...
public class GetProgram {
	...
	public void get_request_is_made() {
		//response = given().when().get("/allPrograms");
		response  = request.get("/allPrograms");
	...
	public void validate_status_code(Integer statusCode) {
	    ...
	    Assert.assertEquals("HTTP/1.1 200 ", statusLine ); //status line validation
...
String statusLine = response.getStatusLine();
		...
		System.out.println("programId is " +pi);//to get the programid from the response
		
		
		String pn = j.getString("programName");
		System.out.println("programName is " +pn);//to get the programName
	...
	v.time(Matchers.lessThan(1000L));
		...
		v.contentType(Matchers.anything(ProgramID));//contain field "ProgramID"
		

v.body("programStatus",Matchers.equalTo("Active"));//Status validation
